...
{
    public abstract class AspNetBoilerPlateWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static AspNetBoilerPlateWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(AspNetBoilerPlateWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<AspNetBoilerPlateDbContext> action)
        {
            using (var context = IocManager.Resolve<AspNetBoilerPlateDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<AspNetBoilerPlateDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<AspNetBoilerPlateDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<AspNetBoilerPlateDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<AspNetBoilerPlateDbContext, Task<T>> func)
        {
            T result;
